...
open class SweetAlert: UIViewController {
    ...
    fileprivate func setupContentView() {
        contentView.backgroundColor = UIColor.init(red: 37/255, green: 37/255, blue: 37/255, alpha: 1.0)
        ...
        contentView.backgroundColor = UIColor.init(hexColor: "394760")
     contentView.layer.borderColor = UIColor(red:57/255, green:71/255, blue:96/255, alpha: 1).cgColor
    ...
    fileprivate func setupTitleLabel() {
        ...
        titleLabel.textColor = UIColor.colorFromRGB(0xFFFFFF)
    }
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            textViewHeight = ceil(rect.size.height) + 35.0                          // Increase the height of TextView.
...
//            }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0xFFFFFF))
        userAction = action
        return self
    }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        if subTitle != nil {
            if title.lowercased().contains("success") || subTitle!.lowercased().contains("success"){
                 self.animatedView = SuccessAnimatedView()
            }
        }
        if title.lowercased().contains("success"){
            ...
            if buttonTitle.isEmpty == false {
                ...
                button.tag = 0
                button.titleLabel?.font =  UIFont(name: (button.titleLabel?.font.familyName)!, size: 16)
                buttons.append(button)
            }
            
            if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
                let button: UIButton = UIButton(type: UIButton.ButtonType.custom)
                button.setTitle("   \(otherButtonTitle ?? "Cancel")   ", for: UIControl.State())
    ...
    override open func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if shareCheck{
        for view1 in self.contentView.subviews{
            if  view1 != subTitleTextView.inputView && view1 != titleLabel.inputView{
                self.view.removeFromSuperview()
                }
            }
        }
